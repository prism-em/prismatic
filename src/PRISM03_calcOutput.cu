...
namespace Prismatic {
	...
	inline void allocatePinnedHostMemory_singlexfer3(Parameters<PRISMATIC_FLOAT_PRECISION> &pars,
		...
		{
			cuda_pars.output_c_ph = new std::complex<PRISMATIC_FLOAT_PRECISION> *[total_num_streams]; // one output array per stream
			for (auto s = 0; s < total_num_streams; ++s) {
				cudaErrchk(cudaMallocHost((void **) &cuda_pars.output_c_ph[s],     pars.output_c.get_dimi()   * sizeof(std::complex<PRISMATIC_FLOAT_PRECISION>)));
			}
		}
		else
		{
	...
	inline void allocatePinnedHostMemory_streaming3(Parameters<PRISMATIC_FLOAT_PRECISION> &pars,
		...
		if(pars.meta.saveComplexOutputWave)
		{
			cuda_pars.output_c_ph = new std::complex<PRISMATIC_FLOAT_PRECISION> *[total_num_streams]; // one output array per stream
			for (auto s = 0; s < total_num_streams; ++s) {
				cudaErrchk(cudaMallocHost((void **) &cuda_pars.output_c_ph[s],
				pars.output_c.get_dimi() * sizeof(std::complex<PRISMATIC_FLOAT_PRECISION>)));
			}
		}
		else
		{
	...
	inline void copyToPinnedMemory_singlexfer3(Parameters<PRISMATIC_FLOAT_PRECISION> &pars,
		...
		{
			for (auto s = 0; s < total_num_streams; ++s) {
				memset(cuda_pars.output_c_ph[s], 0, pars.output_c.get_dimi() * sizeof(std::complex<PRISMATIC_FLOAT_PRECISION>));
			}
		}
		else
		{	
	...
	inline void allocateDeviceMemory_singlexfer3(Parameters<PRISMATIC_FLOAT_PRECISION> &pars,
		...
		if(pars.meta.saveComplexOutputWave)
		{
			cuda_pars.integratedOutput_c_ds = new PRISMATIC_CUDA_COMPLEX_FLOAT *[total_num_streams];
		}
		else
		{
		...
		for (auto s = 0; s < total_num_streams; ++s) {
			...
			if(pars.meta.saveComplexOutputWave)
			{
				cudaErrchk(cudaMalloc((void **) &cuda_pars.integratedOutput_c_ds[s], pars.detectorAngles.size() * sizeof(PRISMATIC_CUDA_COMPLEX_FLOAT)));
				cudaErrchk(cudaMemset(cuda_pars.integratedOutput_c_ds[s], 0, pars.detectorAngles.size() * sizeof(PRISMATIC_CUDA_COMPLEX_FLOAT)));
			}
			else
			{
	...
	inline void launchWorkers_singlexfer3(Parameters<PRISMATIC_FLOAT_PRECISION> &pars,
		...
		for (auto t = 0; t < total_num_streams; ++t) {
			...
			PRISMATIC_FLOAT_PRECISION *current_psiIntensity_ds;
			PRISMATIC_FLOAT_PRECISION *current_integratedOutput_ds;
			PRISMATIC_CUDA_COMPLEX_FLOAT *current_integratedOutput_c_ds;
			PRISMATIC_FLOAT_PRECISION *current_output_ph;
			std::complex<PRISMATIC_FLOAT_PRECISION> *current_output_c_ph;
			
			if(pars.meta.saveComplexOutputWave)
			{
				current_integratedOutput_c_ds = cuda_pars.integratedOutput_c_ds[stream_count];
				current_output_c_ph = cuda_pars.output_c_ph[stream_count];
			}
			else
			{
				current_psiIntensity_ds    = cuda_pars.psiIntensity_ds[stream_count];
				current_integratedOutput_ds = cuda_pars.integratedOutput_ds[stream_count];
				// get pointer to output pinned memory
				current_output_ph = cuda_pars.output_ph[stream_count];
			}
			...
			workers_GPU.push_back(
					thread([&pars, GPU_num, stream_count, current_permutedScompact_d, &dispatcher, current_alphaInd_d, current_PsiProbeInit_d, current_qxaReduce_d, current_qyaReduce_d,
								   ...
								   current_integratedOutput_c_ds, current_output_ph, current_output_c_ph,
...
//					cout << "maxGPUMem = " << pars.maxGPUMem << endl;
						...
						while (dispatcher.getWork(Nstart, Nstop)) { // synchronously get work assignment
							while (Nstart < Nstop) {
								...
								if(pars.meta.saveComplexOutputWave)
								{
									buildSignal_c_GPU_singlexfer(pars, ay, ax, current_permutedScompact_d,
										current_PsiProbeInit_d, current_qxaReduce_d,
										current_qyaReduce_d,
										current_yBeams_d, current_xBeams_d, current_alphaInd_d,
										current_psi_ds,
										current_phaseCoeffs_ds, current_y_ds, current_x_ds, 
										current_output_c_ph, current_integratedOutput_c_ds,
...
//					cout << "maxGPUMem = " << pars.maxGPUMem << endl;
				...
				while (dispatcher.getWork(Nstart, Nstop)) { // synchronously get work assignment
					while (Nstart < Nstop) {
						...
						if(pars.meta.saveComplexOutputWave)
						{
							buildSignal_c_GPU_streaming(pars, ay, ax, current_permutedScompact_ds, cuda_pars.permutedScompact_ph,
								...
								current_phaseCoeffs_ds, current_y_ds, current_x_ds,
								current_output_c_ph, current_integratedOutput_c_ds, current_cufft_plan, current_stream,  cuda_pars );
						}
						else
						{
...
//					while (getWorkID(pars, Nstart, Nstop)) { // synchronously get work assignment
	...
	inline void cleanupMemory3(Parameters<PRISMATIC_FLOAT_PRECISION>& pars,
		...
		{
			for (auto s = 0; s < total_num_streams; ++s) {
				cudaErrchk(cudaFreeHost(cuda_pars.output_c_ph[s]));
			}
		}
		else
		{
		...
		for (auto s = 0; s < total_num_streams; ++s) {
			...
			if(pars.meta.saveComplexOutputWave)
			{
				cudaErrchk(cudaFree(cuda_pars.integratedOutput_c_ds[s]));
			}
			else
			{
		...
		if(pars.meta.saveComplexOutputWave)
		{
			delete[] cuda_pars.output_c_ph;
			delete[] cuda_pars.integratedOutput_c_ds;
		}
		else
		{
...
//		 step through global memory accumulating until values have been reduced to BlockSize_numBeams elements in shared memory
	...
	void buildSignal_c_GPU_singlexfer(Parameters<PRISMATIC_FLOAT_PRECISION>&  pars,
		...
		// output calculation result
		size_t write_ay = (pars.meta.arbitraryProbes) ? 0 : ay;
		formatOutput_GPU_c_integrate(pars, psi_ds, alphaInd_d, output_c_ph,
			integratedOutput_c_ds, qyaReduce_d, qxaReduce_d, 0, write_ay, ax, pars.imageSizeReduce[0],
			pars.imageSizeReduce[1], stream, pars.scale);
	}
...
//		// Launch kernel. Block size must be visible at compile time so we use a switch statement
	...
	void buildSignal_c_GPU_streaming(Parameters<PRISMATIC_FLOAT_PRECISION>&  pars,
	                               const size_t& ay,
	                               const size_t& ax,
	                               PRISMATIC_CUDA_COMPLEX_FLOAT *permutedScompact_ds,
	                               const std::complex<PRISMATIC_FLOAT_PRECISION> *permutedScompact_ph,
