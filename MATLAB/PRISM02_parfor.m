...
comp = 0;
progressbar(comp,2);
tmp_numFP = emdSTEM.numFP;
tmp_imageSize = emdSTEM.imageSize;
tmp_beamsIndex = emdSTEM.beamsIndex;
tmp_numPlanes = emdSTEM.numPlanes;
tmp_prop = emdSTEM.prop;
tmp_Scompact = emdSTEM.Scompact;
for a0 = 1:emdSTEM.numberBeams
    for a1 = 1:tmp_numFP
        % Initialize plane wave
        psi = zeros(tmp_imageSize);

%         psi(:) = 0;
        psi(tmp_beamsIndex(a0)) = 1;
        psi = ifft2(psi);
        
        % Propgate through all potential planes
        for a2 = 1:tmp_numPlanes
            psi = ifft2(fft2(psi.*trans(:,:,a2,a1)).*tmp_prop);
        end
