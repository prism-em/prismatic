...
files = [f for f in os.listdir(os.getcwd()) if "Xtilt" in f]

for i, f in  enumerate(files):
	img = readMRC(f)
	print("Reading {}".format(f))
	if i == 0:
		stack = np.zeros((img.shape[0], img.shape[1], len(files)), dtype=float)
	stack[:, :, i]  = np.squeeze(img)
scipy.io.savemat('images.mat',{"stack":stack})
